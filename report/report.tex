...
\begin{abstract}
An algorithm classifying ECG heartbeats as normal (N) or ventricular ectopic (V) was implemented using linear norm strategies. Performance was evaluated on the MIT-BIH Arrhythmia Database~\cite{moody2001mitbih}, showing high accuracy.
\end{abstract}
...
\begin{itemize}
    ...
    \[
        \theta_s^\text{final} = median_s + k^* * \sigma_s
    \]
    ...
    \[
        v_{i,j} =
        \begin{cases}
        V, & \text{if } d_j(q_i, \hat{q}) > T_j \\
        V, & 
        \begin{aligned}
            & \text{if is\_early and has\_pause and } \\
            & d_j(q_i, \hat{q}) > 0.7 \, T_j
        \end{aligned} \\[1mm]
...
To develop this algorithm, I used the MIT-BIH Arrhythmia Database~\cite{moody2001mitbih} from PhysioNet~\cite{goldberger2000physiobank}. The efficiency of the algorithm was then defined with the average sensitivity (Se), average positive predictivity (+P) and average specificity (Sp) threw the whole database.
\[
